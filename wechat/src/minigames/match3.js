...
(function (global) {
  ...
  Game.prototype._newCell = function (t, spawnOff) {
    var cell = {
      type: (t == null ? rnd(this.names.length) : t),
      sp: SP.NONE,
      dirt: false, knot: 0,
  ...
  Game.prototype._initBoard = function (attempt) {
    ...
    for (var r = 0; r < this.rows; r++) {
      ...
      for (var c = 0; c < this.cols; c++) {
        var t;
        var guard = 0;
        do { t = rnd(this.names.length); guard++; }
        while (this._wouldMatchAt(g, r, c, t) && guard < 30);
        if (this._wouldMatchAt(g, r, c, t)) {
          for (var candidate = 0; candidate < this.names.length; candidate++) {
            if (!this._wouldMatchAt(g, r, c, candidate)) { t = candidate; break; }
          }
        }
        var cell = this._newCell(t);
        if (this.kind === 'CLEAN' && Math.random() < 0.40) { cell.dirt = true; this.initialDirt++; }
        if (this.kind === 'GROOM' && Math.random() < 0.32) { cell.knot = 2; this.initialKnot += 2; }
        g[r][c] = cell;
      }
    }
    this.grid = g;
  ...
  Game.prototype._forceOpeningMove = function () {
    ...
    for (var r = 0; r < pattern.length && r < this.rows; r++) {
      for (var c = 0; c < pattern[r].length && c < this.cols; c++) {
        ...
        if (cell) cell.type = pattern[r][c] % this.names.length;
      }
    }
  ...
  Game.prototype._removeAndGravity = function () {
    ...
    for (c = 0; c < this.cols; c++) {
      ...
      for (r = writeR; r >= 0; r--) {
        ...
        if (this.kind === 'CLEAN' && Math.random() < 0.24) { nc.dirt = true; this.initialDirt++; }
        if (this.kind === 'GROOM' && Math.random() < 0.22) { nc.knot = 2; this.initialKnot += 2; }
        this.grid[r][c] = nc;
      }
    }
  ...
  Game.prototype.canUseItem = function (id) {
    return !this.finished && this.phase === 'idle' && this.energy >= this.itemCost(id);
  };
