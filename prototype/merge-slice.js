(function () {
  ...
  var ART = '../wechat/assets/art/';
  var state, selected = null, drag = null, modal = null, toastTimer, adTimer, careTimer = null, routeSheetOpen = false;
  ...
  var SIDE_REWARD_FAMILIES = ['groom','play'];
  var ORDERS = [
  ...
  function load() {
    ...
    if (raw) { try { state = normalize(JSON.parse(raw)); depositPendingRewards(); save(); return; } catch (e2) {} }
    state = fresh(); save();
  }
  ...
  function sourceLabel(fam) {
    if (fam === 'groom') return '来源：梳理挑战';
  ...
  function merge(a,b) {
    ...
    state.grid[b].tier++; state.grid[b].name=F[state.grid[b].family].items[state.grid[b].tier-1]; state.grid[a]=null; selected=null; var delivered=depositPendingRewards(); save(); render(); toast('合成成功：'+state.grid[b].name+(delivered?' · 已自动入盘暂存素材 ×'+delivered:''));
  }
  ...
  function depositPendingSideRewards() {
    var deposited=0, slots=emptyIndices();
    SIDE_REWARD_FAMILIES.forEach(function (fam) {
  ...
  function depositPendingRewards() {
    return depositPendingHerbs()+depositPendingSideRewards();
  }
  function pendingSideCount() {
    return SIDE_REWARD_FAMILIES.reduce(function (sum, fam) { return sum+(state.pendingSideRewards[fam]||0); }, 0);
  }
  ...
  function closeModal() { clearInterval(adTimer); adTimer=null; clearInterval(careTimer); careTimer=null; modal=null; routeSheetOpen=false; var r=q('modal-root'); if(r)r.innerHTML=''; }
  ...
  function unlockCell() { if(state.unlockedCells>=TOTAL){toast('棋盘已经全部开放');return;}if(state.jade<25){toast('暖玉不足，需要 25 暖玉');return;}state.jade-=25;state.unlockedCells=Math.min(TOTAL,state.unlockedCells+1);var delivered=depositPendingRewards();save();closeModal();render();toast('已解锁 1 个棋盘格（'+state.unlockedCells+'/'+TOTAL+'）'+(delivered?' · 暂存素材已自动入盘 ×'+delivered:'')); }

  function renderYardGoals() {
    var root=q('yard-goals'); if(!root)return; ensureDaily(); var list=[['herb','访问药圃','点击左侧药圃'],['soothe','安抚穷奇','点击庭院中的穷奇'],['care','完成互动','完成一次梳理或陪玩']]; root.innerHTML=list.map(function(a){var done=!!state.yardGoals[a[0]];return '<div class="yard-goal '+(done?'done':'')+'"><strong>'+(done?'✓ ':'')+a[1]+'</strong><span>'+a[2]+'</span></div>';}).join(''); var all=state.yardGoals.herb&&state.yardGoals.soothe&&state.yardGoals.care; var pending=[];if(state.pendingHerbRewards)pending.push('药材×'+state.pendingHerbRewards);SIDE_REWARD_FAMILIES.forEach(function(fam){if(state.pendingSideRewards[fam])pending.push(F[fam].name+'×'+state.pendingSideRewards[fam]);}); var reward=q('yard-goal-reward'); if(reward)reward.textContent=pending.length?'暂存素材：'+pending.join('、')+' · 空出格子后自动入盘': '完成得 +8 体力 · +1 净化刷 · 随机药材'; var b=q('claim-yard-goal');b.disabled=!all||state.yardGoals.claimed;b.textContent=state.yardGoals.claimed?'今日目标奖励已领取':(all?'领取今日奖励':'完成 '+[state.yardGoals.herb,state.yardGoals.soothe,state.yardGoals.care].filter(function(x){return !x;}).length+' 项目标'); }
  function markYardGoal(key) { ensureDaily(); if(!state.yardGoals[key]){state.yardGoals[key]=true;save();renderYardGoals();} }
  function claimYardGoals() { ensureDaily(); var all=state.yardGoals.herb&&state.yardGoals.soothe&&state.yardGoals.care;if(!all||state.yardGoals.claimed)return;state.yardGoals.claimed=true;addEnergy(8);state.cleanTools++;state.pendingHerbRewards++;var delivered=depositPendingRewards();if(delivered)toast('庭院奖励：+8 体力、+1 净化刷、素材已入盘 ×'+delivered);else toast('庭院奖励：+8 体力、+1 净化刷；素材已暂存，空出格子后自动入盘');save();render(); }
  function renderYard() {
  ...
  /* The two care activities deliberately use different interaction grammars.
     Groom is a swipe-first match board; play is a pair-link board. Both use
  ...
  function careSetHud(){if(!modal)return;if(q('care-score'))q('care-score').textContent=String(Math.round(modal.score||0));if(q('combo'))q('combo').textContent='×'+(modal.combo||0);if(q('care-progress'))q('care-progress').textContent=modal.type==='groom'?Math.min(24,modal.goal)+'/24':(modal.playMatched||0)+'/'+PLAY_PAIRS;if(q('care-time'))q('care-time').textContent=modal.type==='play'?Math.max(0,modal.timeLeft)+'s':'本局';}
  function careFloat(text){var board=q('modal-root')&&q('modal-root').querySelector('.care-board');if(!board)return;var el=document.createElement('span');el.className='care-float';el.textContent=text;board.appendChild(el);setTimeout(function(){if(el.parentNode)el.parentNode.removeChild(el);},650);}
  function careDrawLink(root){if(!modal||!modal.linkPath||!root)return;var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('class','care-link-layer');svg.setAttribute('viewBox','0 0 '+root.clientWidth+' '+root.clientHeight);var boardRect=root.getBoundingClientRect(),points=modal.linkPath.map(function(i){var cell=root.querySelector('[data-care='+i+']');if(!cell)return null;var rect=cell.getBoundingClientRect();return (rect.left-boardRect.left+rect.width/2)+','+(rect.top-boardRect.top+rect.height/2);}).filter(Boolean);if(points.length>1){var line=document.createElementNS('http://www.w3.org/2000/svg','polyline');line.setAttribute('points',points.join(' '));line.setAttribute('class','care-link-line');svg.appendChild(line);root.appendChild(svg);}}
  function careBindGroomSwipe(b){b.addEventListener('pointerdown',function(e){if(!modal||modal.busy)return;b.__careStart={x:e.clientX,y:e.clientY};b.__careSwipe=false;try{b.setPointerCapture(e.pointerId);}catch(err){}});b.addEventListener('pointerup',function(e){var start=b.__careStart;if(!start){return;}b.__careStart=null;var distance=Math.hypot(e.clientX-start.x,e.clientY-start.y);if(distance<14)return;b.__careSwipe=true;var target=document.elementFromPoint(e.clientX,e.clientY),other=target&&target.closest&&target.closest('[data-care]');if(!other){toast('把素材滑向相邻格');return;}var a=+b.dataset.care,c=+other.dataset.care;if(!careAdjacent(a,c)){toast('只能滑向相邻的素材');return;}if(!modal||modal.busy||!modal.board[a]||!modal.board[c])return;modal.selected=null;careSwap(a,c);modal.busy=true;careResolve(a,c);});b.addEventListener('pointercancel',function(){b.__careStart=null;});b.addEventListener('click',function(){if(b.__careSwipe){b.__careSwipe=false;return;}careTap(+b.dataset.care);});}
  function careRender(){var root=q('modal-root').querySelector('.care-board');if(!root||!modal)return;root.classList.toggle('care-play-board',modal.type==='play');root.innerHTML=modal.board.map(function(x,i){if(!x)return '<button class="care-empty" data-care="'+i+'" disabled></button>';var selected=modal.selected===i,hint=modal.hintPair&&modal.hintPair.indexOf(i)>=0,hit=modal.linkPath&&modal.linkPath.indexOf(i)>=0;return '<button class="care-tile '+(selected?'care-selected ':'')+(hint?'care-hint ':'')+(hit?'care-link-hit ':'')+'" data-care="'+i+'"><img src="'+iconPath(x)+'" alt="'+F[x.family].items[Math.max(0,(x.tier||1)-1)]+'"><b>'+x.tier+'</b></button>';}).join('');root.querySelectorAll('[data-care]').forEach(function(b){if(modal.type==='groom')careBindGroomSwipe(b);else b.addEventListener('click',function(){careTap(+b.dataset.care);});});careSetHud();if(modal.linkPath)window.requestAnimationFrame(function(){careDrawLink(root);});}
  function careFinish(){if(!modal||modal.completed)return;modal.completed=true;clearInterval(careTimer);careTimer=null;var rewardFamily=modal.type==='groom'?'groom':'play';state.pendingSideRewards[rewardFamily]=(state.pendingSideRewards[rewardFamily]||0)+1;var deposited=depositPendingRewards();state.minigameWins++;state.beast.trust=Math.min(100,state.beast.trust+8);state.beast.heal=Math.min(100,state.beast.heal+5);state.beast.bond=Math.min(5,state.beast.bond+1);state.lastStory=modal.type==='groom'?'梳子停下时，穷奇的鬃毛终于顺了下来。':'风铃飞回掌心，穷奇在庭院里追着你跑了一圈。';markYardGoal('care');save();var score=Math.round(modal.score||0);setTimeout(function(){closeModal();render();toast('互动完成 · '+F[rewardFamily].name+'素材'+(deposited?'已入盘 ×'+deposited:'已暂存，空出格子后自动入盘')+' · 得分 '+score);},420);}
  function careResolve(a,b){var found=careFindMatches(),hit=found.hit;if(!hit.size){careSwap(a,b);modal.combo=0;modal.busy=false;modal.selected=null;careRender();toast('这次交换没有形成连线');return;}var count=hit.size,bonus=count>=5?260:(count>=4?120:0);modal.score+=Math.round(count*100*(1+modal.combo*0.25))+bonus;modal.combo++;modal.goal+=count;hit.forEach(function(i){modal.board[i]=null;});modal.turn++;careDrop();modal.busy=false;modal.selected=null;careRender();careFloat('+'+(Math.round(count*100*(1+(modal.combo-1)*0.25))+bonus));if(modal.goal>=24)careFinish();}
  function carePlayFindPath(a,b){if(!modal||!modal.board[a]||!modal.board[b]||!carePlaySame(modal.board[a],modal.board[b]))return null;var queue=[{i:a,dir:-1,turns:0,path:[a]}],seen={};while(queue.length){var cur=queue.shift();for(var d=0;d<4;d++){var turns=cur.turns+(cur.dir<0||cur.dir===d?0:1);if(turns>2)continue;var row=Math.floor(cur.i/PLAY_COLS),col=cur.i%PLAY_COLS,dr=[-1,0,1,0][d],dc=[0,1,0,-1][d],nr=row+dr,nc=col+dc;if(nr<0||nr>=PLAY_ROWS||nc<0||nc>=PLAY_COLS)continue;var ni=nr*PLAY_COLS+nc;if(ni!==b&&modal.board[ni])continue;var key=ni+'|'+d;if(seen[key]!=null&&seen[key]<=turns)continue;seen[key]=turns;var path=cur.path.concat(ni);if(ni===b)return path;queue.push({i:ni,dir:d,turns:turns,path:path});}}return null;}
  function carePlayFindPair(){for(var i=0;i<modal.board.length;i++)if(modal.board[i])for(var j=i+1;j<modal.board.length;j++)if(modal.board[j]&&carePlaySame(modal.board[i],modal.board[j])&&carePlayFindPath(i,j))return [i,j];return null;}
  function carePlayResolve(a,b,path){modal.busy=true;modal.selected=null;modal.linkPath=path;careRender();setTimeout(function(){if(!modal||modal.type!=='play')return;modal.board[a]=null;modal.board[b]=null;modal.linkPath=null;modal.playMatched++;modal.score+=150+modal.combo*50;modal.combo++;careRender();careFloat('+'+(150+(modal.combo-1)*50));if(modal.playMatched>=PLAY_PAIRS)careFinish();else if(!carePlayFindPair()){modal.combo=0;careRender();toast('素材暂时没有可连的路线，试试「风铃散射」');}else{modal.busy=false;}},280);}
  function carePlayTap(i){if(!modal||modal.busy||!modal.board[i])return;if(modal.selected===null){modal.selected=i;careRender();return;}if(modal.selected===i){modal.selected=null;careRender();return;}var a=modal.selected,path=carePlayFindPath(a,i);if(!path){modal.selected=null;modal.combo=0;careRender();toast('这两枚素材暂时连不上');return;}carePlayResolve(a,i,path);}
  function careShufflePlay(){var tiles=modal.board.filter(Boolean);careShuffle(tiles);for(var i=0;i<modal.board.length;i++)modal.board[i]=tiles[i]||null;modal.selected=null;modal.hintPair=null;careRender();toast('风铃散射：素材重新排布');}
  function careUseTool(tool){if(!modal||modal.busy||!modal.tools||!modal.tools[tool])return;modal.tools[tool]--;if(modal.type==='groom'){var idx=modal.selected!==null&&modal.board[modal.selected]?modal.selected:modal.board.findIndex(Boolean);if(tool==='brush'&&idx>=0){modal.board[idx]=null;modal.selected=null;modal.goal++;modal.combo++;modal.score+=90;modal.turn++;careDrop();careRender();careFloat('+90 · 顺毛');if(modal.goal>=24)careFinish();}else if(tool==='dew'){var chosen=modal.selected!==null&&modal.board[modal.selected]?modal.board[modal.selected].family:null;if(!chosen){var counts={};modal.board.forEach(function(x){if(x)counts[x.family]=(counts[x.family]||0)+1;});chosen=Object.keys(counts).sort(function(a,b){return counts[b]-counts[a];})[0];}var removed=0;modal.board.forEach(function(x,i){if(x&&x.family===chosen){modal.board[i]=null;removed++;}});modal.selected=null;modal.goal+=removed;modal.combo++;modal.score+=removed*115;modal.turn++;careDrop();careRender();careFloat('+'+(removed*115)+' · 舒缓');if(modal.goal>=24)careFinish();}}else if(modal.type==='play'){if(tool==='hint'){var pair=carePlayFindPair();if(!pair){toast('已经没有可连的素材');return;}modal.hintPair=pair;careRender();var current=modal;setTimeout(function(){if(modal===current){modal.hintPair=null;careRender();}},1200);toast('灵犀提示：找到一对可连素材');}else if(tool==='shuffle')careShufflePlay();}}
  function careClockTick(){if(!modal||modal.type!=='play')return;modal.timeLeft--;careSetHud();if(modal.timeLeft<=0){clearInterval(careTimer);careTimer=null;modal.busy=true;careRender();setTimeout(function(){if(modal&&modal.type==='play'&&!modal.completed){var score=Math.round(modal.score||0);closeModal();toast('陪玩结束 · 本局得分 '+score+'，再来一次试试');}},500);}}
  function careTap(i){if(!modal||modal.busy)return;if(modal.type==='play'){carePlayTap(i);return;}if(modal.selected===null){modal.selected=i;careRender();return;}if(modal.selected===i){modal.selected=null;careRender();return;}if(!careAdjacent(modal.selected,i)){modal.selected=i;careRender();toast('只能交换相邻的素材');return;}var a=modal.selected;careSwap(a,i);modal.busy=true;careResolve(a,i);}
  ...
  function openCare(type){clearInterval(careTimer);careTimer=null;var play=type==='play';modal={type:type,turn:0,score:0,goal:0,combo:0,busy:false,selected:null,completed:false,tools:play?{hint:2,shuffle:1}:{brush:2,dew:1},playMatched:0,timeLeft:45,board:play?carePlayBoard():Array.from({length:CARE_COLS*CARE_ROWS},function(_,i){return careToken(type,i);})};var title=play?'陪穷奇玩风铃':'梳开打结的鬃毛',copy=play?'点击两枚相同素材，路径畅通即可连线收集；45 秒内完成全部配对。':'拖动相邻素材交换；连成三个以上即可收集，滑动越顺，连击与分数越高。',rewardName=play?'陪玩':'梳毛';var sec=modalShell(play?'play-care-modal':'groom-care-modal','<span class="eyebrow">庭院互动 · '+(play?'陪玩挑战':'梳理挑战')+'</span><h2>'+title+'</h2><p>'+copy+'完成互动会推进今日庭院目标，并获得 1 个「'+rewardName+'」1阶素材，自动放入合成棋盘。</p><div class="care-status"><b>得分 <i id="care-score">0</i></b><b>连击 <i id="combo">×0</i></b><b>进度 <i id="care-progress">'+(play?'0/'+PLAY_PAIRS:'0/24')+'</i></b><b>剩余 <i id="care-time">'+(play?'45s':'本局')+'</i></b></div>'+careToolsMarkup(type)+'<div class="care-board"></div><div class="care-legend">合成路线素材会出现在互动棋盘里</div><div id="care-beast" class="care-beast">'+(play?'🎐':'🐾')+'</div>');sec.querySelectorAll('[data-care-tool]').forEach(function(b){b.addEventListener('click',function(){careUseTool(b.dataset.careTool);});});careRender();if(play)careTimer=setInterval(careClockTick,1000);}

  function organize(){var movable=state.grid.filter(function(x){return x&&x.family&&x.kind!=='generator';}).sort(function(a,b){return (b.tier||0)-(a.tier||0);}),i=0;state.grid.forEach(function(x,idx){if(unlocked(idx)&&x&&x.family&&x.kind!=='generator')state.grid[idx]=null;});state.grid.forEach(function(x,idx){if(unlocked(idx)&&!state.grid[idx]&&movable[i])state.grid[idx]=movable[i++];});selected=null;var delivered=depositPendingRewards();save();render();toast('物品已整理，高阶材料排在前面'+(delivered?' · 暂存素材已自动入盘 ×'+delivered:''));}
  ...
  /* Use dataset matching here instead of a numeric attribute selector; the
     latter is invalid CSS in Chromium and breaks the visible link path. */
  function careDrawLink(root){if(!modal||!modal.linkPath||!root)return;var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('class','care-link-layer');svg.setAttribute('viewBox','0 0 '+root.clientWidth+' '+root.clientHeight);var boardRect=root.getBoundingClientRect(),points=modal.linkPath.map(function(i){var cell=Array.from(root.querySelectorAll('[data-care]')).find(function(n){return +n.dataset.care===i;});if(!cell)return null;var rect=cell.getBoundingClientRect();return (rect.left-boardRect.left+rect.width/2)+','+(rect.top-boardRect.top+rect.height/2);}).filter(Boolean);if(points.length>1){var line=document.createElementNS('http://www.w3.org/2000/svg','polyline');line.setAttribute('points',points.join(' '));line.setAttribute('class','care-link-line');svg.appendChild(line);root.appendChild(svg);}}
  /* Final interaction hooks keep tool counts, drag feedback and the link
     resolution state in sync after each animated move. */
  function careSetHud(){if(!modal)return;if(q('care-score'))q('care-score').textContent=String(Math.round(modal.score||0));if(q('combo'))q('combo').textContent='×'+(modal.combo||0);if(q('care-progress'))q('care-progress').textContent=modal.type==='groom'?Math.min(24,modal.goal)+'/24':(modal.playMatched||0)+'/'+PLAY_PAIRS;if(q('care-time'))q('care-time').textContent=modal.type==='play'?Math.max(0,modal.timeLeft)+'s':'本局';var root=q('modal-root');if(root)root.querySelectorAll('[data-care-tool]').forEach(function(button){var key=button.dataset.careTool,left=modal.tools&&modal.tools[key]||0,b=button.querySelector('strong b');if(b)b.textContent='×'+left;button.disabled=!!modal.busy||left<1;});}
  function careBindGroomSwipe(b){b.addEventListener('pointerdown',function(e){if(!modal||modal.busy)return;b.__careStart={x:e.clientX,y:e.clientY};b.__careSwipe=false;b.classList.add('care-dragging');try{b.setPointerCapture(e.pointerId);}catch(err){}});b.addEventListener('pointerup',function(e){var start=b.__careStart;if(!start)return;b.__careStart=null;b.classList.remove('care-dragging');var distance=Math.hypot(e.clientX-start.x,e.clientY-start.y);if(distance<14)return;b.__careSwipe=true;var target=document.elementFromPoint(e.clientX,e.clientY),other=target&&target.closest&&target.closest('[data-care]');if(!other){toast('把素材滑向相邻格');return;}var a=+b.dataset.care,c=+other.dataset.care;if(!careAdjacent(a,c)){toast('只能滑向相邻的素材');return;}if(!modal||modal.busy||!modal.board[a]||!modal.board[c])return;modal.selected=null;careSwap(a,c);modal.busy=true;careResolve(a,c);});b.addEventListener('pointercancel',function(){b.__careStart=null;b.classList.remove('care-dragging');});b.addEventListener('click',function(){if(b.__careSwipe){b.__careSwipe=false;return;}careTap(+b.dataset.care);});}
