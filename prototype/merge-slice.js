(function () {
  ...
  function careResolve(a,b){var found=careFindMatches(),hit=found.hit;if(!hit.size){careSwap(a,b);modal.busy=false;modal.selected=null;careRender();toast('这次交换没有形成连线');return;}modal.score+=hit.size*100*(1+modal.combo);modal.combo++;modal.goal+=hit.size;hit.forEach(function(i){modal.board[i]=null;});q('combo').textContent='×'+modal.combo;q('care-score').textContent=modal.type==='groom'?Math.min(24,modal.goal)+'/24':Math.min(1200,modal.score)+'/1200';careDrop();careRender();modal.busy=false;modal.selected=null;if(!modal.completed&&((modal.type==='groom'&&modal.goal>=24)||(modal.type==='play'&&modal.score>=1200))){modal.completed=true;var rewardFamily=modal.type==='groom'?'groom':'play';state.pendingSideRewards[rewardFamily]=(state.pendingSideRewards[rewardFamily]||0)+1;var deposited=depositPendingRewards();state.minigameWins++;state.beast.trust=Math.min(100,state.beast.trust+8);state.beast.heal=Math.min(100,state.beast.heal+5);state.beast.bond=Math.min(5,state.beast.bond+1);state.lastStory=modal.type==='groom'?'梳子停下时，穷奇的鬃毛终于顺了下来。':'风铃飞回掌心，穷奇在庭院里追着你跑了一圈。';markYardGoal('care');save();setTimeout(function(){closeModal();render();toast('互动完成 · '+F[rewardFamily].name+'素材'+(deposited?'已入盘 ×'+deposited:'已暂存，空出格子后自动入盘'));},350);}}
  ...
  function openCare(type){modal={type:type,turn:0,score:0,goal:0,combo:0,busy:false,selected:null,board:Array.from({length:CARE_COLS*CARE_ROWS},function(_,i){return careToken(type,i);})};var title=type==='groom'?'梳开打结的鬃毛':'陪穷奇玩风铃',rewardName=type==='groom'?'梳毛':'陪玩';modalShell('care-modal','<span class="eyebrow">庭院互动 · '+(type==='groom'?'梳理挑战':'陪玩挑战')+'</span><h2>'+title+'</h2><p>点击一块素材，再点击相邻素材交换；连成三个以上即可收集。完成互动会推进今日庭院目标，并获得 1 个「'+rewardName+'」1阶素材，自动放入合成棋盘。</p><div class="care-status"><b>连击 <i id="combo">×0</i></b><b>目标 <i id="care-score">'+(type==='groom'?'0/24':'0/1200')+'</i></b></div><div class="care-board"></div><div class="care-legend">合成路线素材会出现在互动棋盘里</div><div id="care-beast" class="care-beast">'+(type==='groom'?'🐾':'🎐')+'</div>');careRender();}
