 ...
 * 神兽陪伴闭环契约：
 * 穷奇陪玩 → 梳妆礼物（九尾狐来信）
 * 九尾狐陪玩 → 陪玩礼物（九尾狐成长 + 饕餮来信）
 * 不同神兽玩同一款游戏掉落不同素材；礼物来源与神兽绑定，通用/他兽素材不能冒名交付。
 */
...
check('giftChain 配置完整：11 段链接覆盖全部 12 只神兽', function () {
  ...
  expect(chain[0].from === 'qiongqi' && chain[0].to === 'jiuweihu' && chain[0].care === 'play' && chain[0].family === 'groom',
    '穷奇陪玩 → 九尾狐的梳妆礼物');
  expect(chain[1].from === 'jiuweihu' && chain[1].to === 'taotie' && chain[1].care === 'play' && chain[1].family === 'play',
    '九尾狐陪玩 → 饕餮的陪玩礼物');
  expect(chain[2].from === 'taotie' && chain[2].to === 'dijiang' && chain[2].family === 'food',
    '饕餮陪玩 → 帝江的膳食礼物');
  chain.forEach((link) => {
...
check('同一款游戏按神兽掉落不同素材，且奖励带来源标记', function () {
  assert.strictEqual(Core.careRouteForBeast('qiongqi', 'play').family, 'groom', '穷奇陪玩路线');
  assert.strictEqual(Core.careRouteForBeast('jiuweihu', 'play').family, 'play', '九尾狐陪玩路线');
  assert.strictEqual(Core.careRouteForBeast('taotie', 'play').family, 'food', '饕餮陪玩路线');
  ...
  expect(qiongqiReward.giftFamily === 'groom' && qiongqiReward.giftSourceBeast === 'qiongqi',
    '穷奇陪玩结算标明礼物族与来源');
  expect(qiongqiReward.rewardItems.every((item) => item.family === 'groom' && item.giftSource === 'qiongqi'),
    '穷奇陪玩奖励全部是穷奇来源的梳妆素材');
});

check('九尾狐来信只认穷奇陪玩送出的梳妆礼物，通用素材不能冒名', function () {
  const state = fresh();
  ...
  arrival.requirements.forEach((need) => {
    expect(need.family === 'groom' && need.sourceBeast === 'qiongqi', '来信需求必须是穷奇陪玩产出的梳妆素材');
  });

  /* 无来源的 groom 素材（生成器/测试工厂默认物）不能交付。 */
  state.grid = state.grid.map(() => null);
  state.grid[0] = Core.makeItem('groom', arrival.requirements[0].tier);
  state.grid[1] = Core.makeItem('groom', arrival.requirements[1].tier);
  expect(Core.canDeliver(state, arrival) === false, '通用梳妆素材不能冒名交付九尾狐来信');

  /* 穷奇陪玩 + 同源合成可以交付。 */
  seedOrder(state, arrival);
  expect(Core.canDeliver(state, arrival) === true, '穷奇来源的梳妆素材可以交付九尾狐来信');
...
check('陪伴奖励实际可闭环：穷奇陪玩 → 梳妆 T6 信物路径可达', function () {
  const state = fresh();
  collectRewards(state, 'qiongqi', 'play', 3);
  expect(state.daily.careRewards.play === 3, '穷奇陪玩三局计入设施奖励');
  expect((state.grid || []).concat(state.pendingRewards || []).filter((item) => item && item.family === 'groom' && item.giftSource === 'qiongqi').length > 0,
    '穷奇陪玩奖励进入棋盘');
});
